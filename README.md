...
跨平台（Windows / macOS）的轻量多格式批量压缩桌面应用。本地离线运行，支持多档位质量调节。

当前已支持 **PDF** 压缩，架构为可插拔引擎设计，后续可扩展图片等多格式。
...
- **文字层保留**（PDF）：默认保留文字（可搜索/复制）；极致压缩档位可选「栅格化去除文字层」
...
- **PDF 压缩引擎**：Ghostscript（`pdfwrite` 设备）

## 前置依赖

### Ghostscript（PDF 压缩必需）
...
产物位置：`src-tauri/target/release/bundle/dmg/压缩器_*.dmg`
...
# 4. 克隆并构建
git clone git@github.com:w1lkin/compress.git
cd compress
npm install
npm run tauri build
```

产物位置：`src-tauri/target/release/bundle/msi/压缩器_*.msi`
...
压缩引擎采用可插拔设计：每种格式实现 `CompressionEngine` trait 并注册即可，前端按文件类型自适应渲染档位 UI。当前已实现 PDF（Ghostscript），未来可扩展图片（JPEG/PNG/WebP/AVIF）、视频等格式。

扩展步骤：
1. 在 `src-tauri/src/engine/` 下新增引擎实现（如 `image.rs`）
2. 实现 `CompressionEngine` trait（声明支持扩展名、档位、压缩方法）
3. 在 `engine/mod.rs` 的 `build_registry()` 中注册

## 许可证

本项目代码采用 MIT 许可。Ghostscript 为 AGPL 双许可，本应用通过"运行时调用用户自行安装的 Ghostscript"的方式规避分发合规问题。
